...
ChromeExOAuth.initBackgroundPage = function(oauth_config){
  ...
  if( oauth_config['app_name'] )
	window.chromeExOAuth['app_name'] = oauth_config['app_name'];
  if( oauth_config['callback_page'] )
	window.chromeExOAuth['callback_page'] = oauth_config['callback_page'];

  var url_match = window.chromeExOAuth.callback_page;
  var tabs = {};
  window.chromeExOAuth.iframe.setAttribute('src', url_match);
  window.chromeExOAuth.iframe.setAttribute('width', '320');
  window.chromeExOAuth.iframe.setAttribute('height', '100%');
  window.chromeExOAuth.iframe.setAttribute('frameborder', '0');
  window.chromeExOAuth.iframe.setAttribute('scrolling', 'yes');
  window.chromeExOAuth.iframe.addEventListener('load', function(){
	var url = window.chromeExOAuth.iframe.contentWindow.window.location.href;
	if (url &&
        url.substr(0, url_match.length) === url_match &&
        window.chromeExOAuthRequestingAccess == false) {
...
ChromeExOAuth.prototype.authorize = function(callback) {
  if (this.hasToken()) {
	...
	//window.chromeExOAuth.iframe.setAttribute('src', this.callback_page);
  }
};
...
ChromeExOAuth.initCallbackPage = function() {
  var background_page = window.parent;
...
ChromeExOAuth.prototype.onRequestToken = function(callback, xhr) {
  if (xhr.readyState == 4) {
    if (xhr.status == 200) {
      ...
      //callback(url);
	  // Переходим на страницу с данными авторизации
	  window.location.href = url;
    } else {
